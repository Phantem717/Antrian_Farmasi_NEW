...
const MissQueue_g = ( {verificationData, pickupData}) => {
...
const socket = getSocket();
  ...
  useEffect(() => {
    const fetchMissedQueues = async () => {
      try {
        socket.on('get_responses_gmcb',(payload)=>{
          const verifData = payload.data.verificationData.filter(
            (queue) => queue.status === "pending_verification" && queue.lokasi === "Lantai 1 GMCB"
          );

          const pickupData = payload.data.pickupData.filter(
            (queue) => queue.status === "pending_pickup_medicine"  && queue.lokasi === "Lantai 1 GMCB"
          );
