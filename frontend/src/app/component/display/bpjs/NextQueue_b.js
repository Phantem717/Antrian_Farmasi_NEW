...
const [times, setTimes] = useState({
  ...
  const processTimeNon = processLengthNon < 3 ? 5 : (Math.floor(processLengthNon * 0.25));
  const processTimeRacik = processLengthRacik < 3 ? 5 : (Math.floor(processLengthRacik * 1));
  const pickupTimeNon = pickupLengthNon < 3 ? 5 : (Math.floor(pickupLengthNon * 0.25));
...
{queue.patient_name}</div>
      ...
      <div className="flex-1 min-w-[300px] bg-gray-100 p-2 rounded-md shadow-md" style={{height: `1120px`}}>
        <p className="text-2xl font-extrabold text-center text-green-700 uppercase">Non-Racikan</p>
        <div className="bg-gray-100 rounded-md p-2" style={{ height: "1000px" }}>
