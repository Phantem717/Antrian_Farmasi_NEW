...
console.log("PILIH TOMBOL CONNECTED");
  ...
  const handleBulkCall = async () => {
    ...
    try {
      ...
      socket.emit('call_queues_verif',{data: selectedQueue2, lokasi: "Lantai 3 GMCB"});
      await Promise.all(
    ...
    try {
      ...
      await Promise.all(
        selectedQueue2.map(async (queue) => {
          ...
          // If status is "processed_verification"
          if (statusType === "processed_verification") {
            if (queue.status_medicine == 'Racikan' ||queue.status_medicine == 'Non - Racikan') {
              await handleBulkPharmacyUpdate(queue.status_medicine);
              await PharmacyAPI.updatePharmacyTask(bookingId, {
